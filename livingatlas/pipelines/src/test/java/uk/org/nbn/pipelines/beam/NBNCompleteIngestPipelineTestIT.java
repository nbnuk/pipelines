 ...
 * current steps in processing. You will likely need to set the following vm options -DZK_PORT=9983
 * -DSOLR_PORT=8983 -DDOCKER_STARTSTOP=false And also copy or link /data/pipelines-shp to
 * /tmp/pipelines-shp
 */
  ...
  public Collection<DynamicTest> testIngestPipeline() throws Exception, Throwable {
    ...
    Map<String, Integer> datasets =
        new HashMap<String, Integer>() {
          {
            put("dr2816", 165);
            put("dr2811", 161); // this has 6 additional records compare to ma
          }
        };
    ...
    tests.add(
        DynamicTest.dynamicTest(
            ...
            () -> {
              assertEquals(
                  Long.valueOf(expectedRecords), SolrUtils.getRecordCount(INDEX_NAME, "*:*"));
